        ...
        var
            ...
            bingGrid = function (data) {
                ...
                var rows = $(jsn).map(function (i, v) {
                    ...
                    VatPaymentTotalAmount += ((v.VAT || 0) * 1); // VAT per Buyer

                    // 
                    return '<tr><td>' + (i + 1) + '</td><td>' + v.AuctionName + '</td><td>' + v.BuyerName + '</td><td>' + numeral(v.FinalAmount).format('0,0.00') + '</td><td>' +
                        numeral(dhsWithCommision).format('0,0.00') + '<a href="payinvoicepaymentspendingprint.aspx#disc-modal" class="hidden-print pull-left add-discount" title="' + numeral(v.DiscountAmount).format('0') + '" data-note="' + (v.DiscountNotes ? v.DiscountNotes : '') + '" data-id="' + v.PayInvoicePaymentsID + '">' + (v.DiscountAmount * 1 > 0 ? numeral(v.DiscountAmount).format('0') : '+خصم') + '</a></td></tr>';
                ...
                var netAmount = ((invTotalDhs * 1) + (VatPaymentTotalAmount * 1));
                ...
                if (invTotal > 0) {
                    $('.totalPaymentTotalDhs').text(numeral(invTotalDhs).format('0,0.00'));
