...
var
    _id = commonManger.getQueryStrs()['id'],
    gridId = 'listItems',
    oTable = $('#' + gridId).DataTable({
        "sDom": "<'row'>t<'row'>",
        searching: false,
        retrieve: true, paging: false,
        destroy: true, sort: false
    }),
    bindmyData = function (data) {
        ...
        if (jsn1 !== undefined) {
            $(jsn1).each(function (i, item) {
                ...
                __list.push(item.UserFullName, numeral(item.DefaultSalary).format('0,0'),
                    numeral(item.SalaryIncrease).format('0,0'), numeral(item.Housing).format('0,0'),
                    numeral(item.Travel).format('0,0'),
                    numeral(item.Perks).format('0,0'),
                    numeral(item.Other).format('0,0'),
                    numeral(item.TotalSum).format('0,0'),
                    numeral(item.Deduct).format('0,0'),
                    numeral(item.RepayAmount).format('0,0'),
                    numeral(item.Total).format('0,0'), ''); //, item.Notes
        ...
        if (jsn !== undefined) {
            ...
            $('#VAT').text(numeral(jsn.VAT).format('0,0'));
    ...
    getSalaryFile = function () {
        ...
        if (_id !== undefined && _id > 0) {
            var functionName = "Salaries_GetDetails",
                dto = { 'actionName': functionName, 'value': _id };


            dataService.callAjax('Post', JSON.stringify(dto),
                sUrl + 'GetData', bindmyData, commonManger.errorException);
        }
    };
