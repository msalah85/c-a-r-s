        ...
        var
            ...
            fillDataTableList = function () {
                var pTable = $('#' + gridId).DataTable({
                    ...
                    "bDestroy": true,
                    "iDisplayLength": 100,
                    "sAjaxSource": 'BankBalancesView.aspx/LoadData',
                    ...
                    "fnServerData": function (sSource, aoData, fnCallback) {
                        dataService.callAjax('GET', aoData, sSource, function (data) {
                            ...
                            // clients deposit not reviewed by admin
                            if (jsn3 && jsn3.ClientsAmountNotReviewed * 1 > 0) {
                                // show extra amount html elements.
                                var msg = '<i data-rel="tooltip" title="إيداعات العملاء الغير معتمدة: ' + numeral(jsn3.ClientsAmountNotReviewed).format('0,0.00') + '، إجمالى الرصيد: ' + numeral((jsn2.Balance ? jsn2.Balance * 1 : 0) + (jsn3.ClientsAmountNotReviewed * 1)).format('0,0.00') + ' درهم." class="icon-info-sign red"></i>';
                                

                                // show total deposit of clients not reviewed.
                                $('span.extra').html(msg);
                                // fire tooltip
                    ...
                    "aoColumns": [{
                        ...
                        "bSortable": false,
                        "mData": function (d) {
                            return '<a href="' + ((d.InAmount * 1) > 0 ? 'ClientPaymentsReviewPrint' : 'ReceiptPaymentsPrint') + '.aspx?id=' + d.ID + '">' + d.ID + '</a>';
                        }
                    },
                    {
                        "mDataProp": "Date",
