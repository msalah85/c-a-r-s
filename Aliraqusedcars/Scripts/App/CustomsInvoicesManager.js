...
CustomsInvoicesManager = function () {
    var $customComp = $('select[id$=CustomsCompanyID]'),
        ...
        SaveDataMasterDetails = function (form, fieldsDetails, valuesDetails, actionName) {
            ...
            ParamNames.push('TotalCustoms', 'TotalExpenses', 'TotalAmount', 'TotalCustomsDhs', 'TotalExpensesDhs', 'TotalAmountDhs');
            ParamValues = arrayall[1];
            var custTotal = parseFloat($('#lblCustomTotal').text()), expenTotal = parseFloat($('#lblTotalExpenses').text()), invTotal = parseFloat($('#lblInvoiceTotal').text());

            ParamValues.push(custTotal, expenTotal, invTotal, custTotal * 3.6930, expenTotal * customsRate(), (custTotal * 3.6930) + (expenTotal * customsRate()));
        ...
        expensesGridTotal = function () {
            ...
            $('#lblInvoiceTotalDhs').text(numeral((customTotal * 3.6930) + (totalTtal * customsRate())).format('0')); // total dirham
        },
