    ...
    ClientsManager = function () {
        var
            ...
            enaleControl = function (control, enabled) {
                if (enabled) {
                    $('#' + control).removeAttr('disabled').html('<i class="fa fa-save"></i> ارسـل رسالة');
                } else {
            ...
            workPerform = function () {
                ...
                $('#send-sms').click(function (e) {
                    ...
                    if (message !== '' && mobile !== '') {
                        var url = "/admin-admin-2015/sms-templates.aspx/SendSMS", dtoObject = { phones: mobile, message: message };
                        ...
                        commonManger.showMessage('حقول مطلوبة', 'برجاء التحقق من رقم الجوال وإدخال  نص الرسالة.');
                    }
                });
                ...
                $(document).delegate("#" + gridId + " tbody tr a.click", "click", function (e) {
                    ...
                    if (pos !== null) {
                        if (self.hasClass('edit')) {
                            var title = "بيانات العميــــــل",
                                operation = 'edit',
                                modalDialog = 'ClientModal',
                                aData = oTable.row(pos).data(),
                                formCtrlBinding = function (row) {
                                    $('#txtName').text(row.full_name);
                                    $('#txtPhone').text(row.phone);
                                    $('#phone2').text(row.phone2);
                                    $('#countryCode').text($.trim(row.countryCode));
                                    $('#countryCode2').text($.trim(row.countryCode2));
                                    $('#txtUsername').text(row.user_name);
                                    $('#txtPassword').text(row.user_password);
                                    //$('#Notes').text(row.Notes);
                                    //$('#user_type').text(row.user_type);
            ...
            getUsername = function () {
                $.ajax({
                    ...
                    beforeSend: function () {
                        $(".sinpper").html("<i class='fa fa-spinner fa fa-spin orange bigger-125'></i>");
                    },
                    complete: function () {
                        $(".sinpper").html("");
                    },
            ...
            filllistItems = function () {
                var oTable = $('#' + gridId).DataTable({
                    ...
                    "fnServerParams": function (aoData) {
                        aoData.push({ "name": "funName", "value": 'Jordan_Clients' }); //, { "name": "names", "value": 'type~client~client_char' }, { "name": "values", "value": type + '~' + client + '~' + ((client_search_char !== undefined && client_search_char !== null) ? client_search_char : '') });
                    },
                    ...
                    "aoColumns": [
                        {
                            ...
                            "mData": function (d) {
                                ...
                                if (d.user_type === '1') {
                                    starClient = '<span class="badge badge-yellow" data-rel="tooltip" title="عميل دائم/عقد"><i class="fa fa-asterisk"></i></span>';
                                    starClient += (d.MasterAs && d.MasterAs > 0) ? '<div class="action-buttons pull-left"><a data-cid="' + d.ClientID + '" data-rel="tooltip" title="حسابات فرعية" href="javascript:void(0);" class="blue bigger-150 show-details-btn"><i class="ace-icon fa fa-chevron-down"></i></a></div>' : '';
                                }

                                return '<a data-rel="tooltip" href="Client.aspx?id=' + d.ClientID + '" title="سيارات العميل">' + d.full_name + '</a> ' + starClient;
                            }
                        },
                        {
                            "mDataProp": "phone",
                        ...
                        },
                        {
                            ...
                            "bSortable": false,
                            "mData": function (oObj) {
                                return '<div class="btn-group"><button data-toggle="dropdown" class="btn btn-small btn-info dropdown-toggle">اخـتـر <i class="fa fa-angle-down fa fa-on-right"></i></button>\
                                    <ul class="dropdown-menu pull-right">\
                                        <li>\
                                            <a href="Client.aspx?id=' + oObj.ClientID + '">سيارات العميل</a>\
                                        </li>\
                                        <li>\
                                            <a data-cid="' + oObj.ClientID + '" class="edit click" data-toggle="modal" data-target="#ClientModal" href="#ClientModal">عرض بيانات العميل</a>\
                                        </li>\
                                        <li>\
