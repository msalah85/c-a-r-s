...
public partial class ETA_ExpenseTypes : FactshMasterPage
{
    #region "Event Handler"
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack) // reset cache
            HttpContext.Current.Cache.Remove("ExpenseTypes");
    }
    ...
    public static object GetExpenseTypes()
    {
        ...
        if (HttpContext.Current.Cache["ExpenseTypes"] == null)
        {
            categoryList = new ExpenseTypesManager().GetExpenseTypes(" AND GroupName IS NULL ");
            HttpContext.Current.Cache.Insert("ExpenseTypes", categoryList, null, Cache.NoAbsoluteExpiration, TimeSpan.FromMinutes(10));
        }
        else
        {
            categoryList = HttpContext.Current.Cache["ExpenseTypes"] as List<ExpenseTypes_SelectRowResult>;
        }
